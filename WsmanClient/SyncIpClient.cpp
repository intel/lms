...
{	
	...
	WSMAN_DEBUG("GetNetworkData Network Settings::	DhcpEnabled=%d IpAddress=%C SubnetMask=%C DefaultGateway=%C\n", DHCPEnabled, IPAddress.c_str(), SubNet.c_str(), GateWay.c_str());
	WSMAN_DEBUG("GetNetworkData DNS Settings::		PrimaryDNS %C, SecondaryDNS %C\n", DNSAddress1.c_str(), DNSAddress2.c_str());
	return true;
}
...
bool SyncIpClient::GetSharedStaticIpState(bool* SharedStaticIpState)
{
	if (!InitEPS())
	{
		WSMAN_ERROR("GetSharedStaticIpState Init return false!\n");
		return false;
	}
